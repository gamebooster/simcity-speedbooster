...
namespace SimCitySpeedBooster {
  public partial class Main : Form {
    ...
    internal Main() {
      ...
      _increaseHotkey.Pressed +=
        (sender, args) => { if (speedNumeric.Value + 1 <= speedNumeric.Maximum) speedNumeric.Value += 1; };
      _increaseHotkey.Register(this);

      _decreaseHotkey = new Hotkey(Settings.Default.decreaseHotkey);
      _decreaseHotkey.Pressed +=
        (sender, args) => { if (speedNumeric.Value - 1 >= speedNumeric.Minimum) speedNumeric.Value -= 1; };
    ...
    private void SearchTimerTick(object sender, EventArgs e) {
      ...
      Process[] processes = Process.GetProcessesByName("SimCity");
      ...
      IntPtr address = _processMemory.FindPattern(
        new byte[] {
